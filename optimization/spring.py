...
# Minimum initial winding (turns) to reach target speed (includes drivetrain efficiency and safety factor)
# From energy conservation:  eta * 0.5 * k_rad * theta_min_rad^2  =  0.5 * m * v_target^2 * safety_factor
k_theta_rad = k_theta * (180.0 / np.pi)          # N·m/rad
min_theta_rad = target_speed_ms * np.sqrt(total_mass_kg * safety_factor / (drivetrain_eff * k_theta_rad))
min_theta_deg = np.degrees(min_theta_rad)
min_turns_to_target = min_theta_deg / 360.0
